...
describe('events', () => {
  describe('formatBuild', () => {
    ...
    test.each([undefined, 'Fnord'])('expect error on invalid character - %s', (character: string | undefined) => {
      const invalidBuild = { ...buildKiller, character }
      expect(() => formatBuild(invalidBuild)).toThrow()
    })

    test.each([undefined, 'Fnord'])('expect error on invalid addon1, killer - %s', (addon1) => {
      const invalidBuild = { ...buildKiller, addon1 }
      expect(() => formatBuild(invalidBuild)).toThrow()
    })

    test.each([undefined, 'Fnord'])('expect error on invalid addon2, killer - %s', (addon2) => {
      const invalidBuild = { ...buildKiller, addon2 }
      expect(() => formatBuild(invalidBuild)).toThrow()
    })

    test.each([undefined, 'Fnord'])('expect error on invalid offering, killer - %s', () => {
      const invalidBuild = { ...buildKiller, offering: undefined }
      expect(() => formatBuild(invalidBuild)).toThrow()
    })

    test.each([undefined, 'Fnord'])('expect error on invalid perk1, killer - %s', () => {
      const invalidBuild = { ...buildKiller, perk1: undefined }
      expect(() => formatBuild(invalidBuild)).toThrow()
    })

    test.each([undefined, 'Fnord'])('expect error on invalid perk2, killer - %s', () => {
      const invalidBuild = { ...buildKiller, perk2: undefined }
      expect(() => formatBuild(invalidBuild)).toThrow()
    })

    test.each([undefined, 'Fnord'])('expect error on invalid perk3, killer - %s', () => {
      const invalidBuild = { ...buildKiller, perk3: undefined }
      expect(() => formatBuild(invalidBuild)).toThrow()
    })

    test.each([undefined, 'Fnord'])('expect error on invalid perk4, killer - %s', () => {
    ...
    test.each([undefined, 'Fnord'])('expect error on invalid item - %s', (item) => {
      const invalidBuild = { ...buildSurvivor, item }
      expect(() => formatBuild(invalidBuild)).toThrow()
    })

    test.each([undefined, 'Fnord'])('expect error on invalid addon1, survivor - %s', (addon1) => {
      const invalidBuild = { ...buildSurvivor, addon1 }
      expect(() => formatBuild(invalidBuild)).toThrow()
    })

    test.each([undefined, 'Fnord'])('expect error on invalid addon2, survivor - %s', (addon2) => {
    ...
    test.each([undefined, 'Fnord'])('expect error on invalid offering, survivor - %s', () => {
      const invalidBuild = { ...buildSurvivor, offering: undefined }
      expect(() => formatBuild(invalidBuild)).toThrow()
    })

    test.each([undefined, 'Fnord'])('expect error on invalid perk1, survivor - %s', () => {
      const invalidBuild = { ...buildSurvivor, perk1: undefined }
      expect(() => formatBuild(invalidBuild)).toThrow()
    })

    test.each([undefined, 'Fnord'])('expect error on invalid perk2, survivor - %s', () => {
      const invalidBuild = { ...buildSurvivor, perk2: undefined }
      expect(() => formatBuild(invalidBuild)).toThrow()
    })

    test.each([undefined, 'Fnord'])('expect error on invalid perk3, survivor - %s', () => {
      const invalidBuild = { ...buildSurvivor, perk3: undefined }
      expect(() => formatBuild(invalidBuild)).toThrow()
    })

    test.each([undefined, 'Fnord'])('expect error on invalid perk4, survivor - %s', () => {
  ...
  describe('extractBuildFromEvent', () => {
    ...
    test('expect build from event', async () => {
      const result = await extractBuildFromEvent(event)
    ...
    test('expect build to be formatted', async () => {
      ...
      const result = await extractBuildFromEvent(tempEvent)
      expect(result).toEqual(expect.objectContaining(buildKiller))
    })
  })
