...
import eventJson from '@events/patch-channel.json'
import { patchChannelHandler } from '@handlers/patch-channel'
...
describe('patch-channel', () => {
  ...
  describe('patchChannelHandler', () => {
    test('expect INTERNAL_SERVER_ERROR on getUserFromEvent reject', async () => {
      ...
      const result = await patchChannelHandler(event)
    ...
    test('expect setChannelById called with updated object', async () => {
      await patchChannelHandler(event)
